...
public class TaskController(TaskService taskService) : ControllerBase
{
    ...
    public async Task<ActionResult<PageResponseDto<TaskResponseDto>>> GetAll([FromRoute] long projectId,
        ...
        [FromQuery] SortOptionsDto? sortOptionsDto, [FromQuery] PageOptionsDto? pageOptionsDto, CancellationToken cancellationToken)
    {
        return Ok(await taskService.GetAllAsync(projectId, taskFilterDto, sortOptionsDto, pageOptionsDto, cancellationToken));
    }

    [HttpGet("{taskId:long}")]
    public async Task<ActionResult<TaskResponseDto>> GetById([FromRoute] long projectId, [FromRoute] long taskId, CancellationToken cancellationToken)
    {
    ...
    {
        var newTask = await taskService.CreateAsync(projectId, taskCreateRequestDto, cancellationToken);
    ...
    public async Task<ActionResult<TaskResponseDto>> Update([FromRoute] long projectId, [FromRoute] long taskId,
        [FromBody] TaskUpdateRequestDto taskUpdateRequestDto, CancellationToken cancellationToken)
    {
        return Ok(await taskService.UpdateAsync(projectId, taskId, taskUpdateRequestDto, cancellationToken));
    }
    ...
    public async Task<ActionResult<TaskResponseDto>> UpdateStatus([FromRoute] long projectId, [FromRoute] long taskId,
        [FromBody] TaskUpdateStatusRequestDto taskUpdateStatusRequestDto, CancellationToken cancellationToken)
    {
        return Ok(await taskService.UpdateStatusAsync(projectId, taskId, taskUpdateStatusRequestDto, cancellationToken));
    }

    [HttpDelete("{taskId:long}")]
    public async Task<ActionResult> Delete([FromRoute] long projectId, [FromRoute] long taskId, CancellationToken cancellationToken)
    {
